...
if(mysqli_num_rows($query) == 0){
   echo "No articles found";
	...
	while($row = mysqli_fetch_assoc($query)){
		 echo" <li class=\"list-group-item\"><a href=\"/route/{$row['tprice_id']}/{$row['linklocation2']}/{$row['linklocation1']}.php\">{$row['tlocation_name2']} - {$row['tlocation_name1']} ({$row['troute_interval']} mins) {$row['tprice_amount']} baht by {$row['vtype_name']}</a></li>"; 
