			...
			if ($_POST){
				...
				if ($act=="add"){
					$sql = "insert into member 
							set mem_contact_firstname = '$firstname'
							  , mem_contact_lastname  = '$lastname'
							  , mem_contact_email     = '$email'
					...
					if ($password!="")
						$updatePassword = "  , mem_password  = '$password'";
		...
		?>
            <table id="dataTable" cellpadding="5" cellspacing="0">
                ...
                <tr class="headTable">
                    ...
                    <a href="../agent/?page=<?php echo $page ?>&total_page=<?php echo $total_page ?>&act=add#frm">Add New</a>
                    <?php } ?>
                    </td>
                </tr>
				<?php 					
                ...
                <tr class="<?php echo $class ?>" onMouseOver="this.className='over'" onMouseOut="this.className='<?php echo $class ?>'">
                    <td class="textCenter"><?php echo str_pad($row["mem_id"],3,"0",STR_PAD_LEFT); ?></td>
                    <td><?php echo $row["mem_contact_firstname"]." ".$row["mem_contact_lastname"] ?></td>
                    <td><?php echo $row["mem_contact_email"] ?></td>
                    <td class="textCenter"><?php echo ShowStatus($row["mem_status"]) ?></td>
                    <td width="6%" class="textCenter"><a href="../agent/?id=<?php echo $row["mem_id"]?>&page=<?php echo $page?>&act=update#frm">Update</a></td>
                    <td width="6%" class="textCenter"><a href="../agent/?id=<?php echo $row["mem_id"]?>&page=<?php echo $page?>&act=delete" onClick="return confirm('Confirm Delete Data ?');">Delete</a></td>
                </tr>
                <?php } ?>
                <tr class="footTable">
                    <td colspan="4">
                ...
                <tr cos class="headTable">
                  <td class="postHead textCenter" colspan="2"><?php
						if ($act == "add"){ 
							...
							$firstname = $row["mem_contact_firstname"];
							$lastname  = $row["mem_contact_lastname"];
							$email 	 = $row["mem_contact_email"];
							$status 	= $row["mem_status"];

							echo "<input type='hidden' name='oldpass' id='oldpass' value='".$row["mem_password"]."'>";	
